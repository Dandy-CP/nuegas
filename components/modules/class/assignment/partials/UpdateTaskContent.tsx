...
import { UpdateAssignmentTask } from '@/service/api/assignment/assignment.mutation';
...
import { Assignment, CreateAssignment } from '@/types/assignment.types';
...
function UpdateTaskContent({
	...
	const [selectedTopic, setSelectedTopic] = useState<Topic | null>({
		name: assignmentData?.topic?.name ?? '',
		topic_id: assignmentData?.topic?.topic_id ?? '',
		class_id: '',
	});
	...
	const { mutateAsync, isPending } = UpdateAssignmentTask(
		{
			assignment_id: assignmentData?.assignments_id ?? '',
		},
		{
			onSuccess() {
				onRefetch();
			...
			onError() {
				toast.error('Error on update assignment');
			},
		}
	);
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				...
				>
					<Toolbar>
						...
						<Typography sx={{ ml: 2, flex: 1 }}>Update Task</Typography>
				...
				<div className='flex flex-row justify-between'>
					...
					<div className='flex h-screen w-1/3 flex-col gap-5 border-l border-l-gray-300 bg-white p-5'>
						<h1 className='text-xl font-semibold'>{taskName}</h1>
